...
0.08 & 0.494 & 0.976 & 0.046 & 1.928 & 5600 & 3.084
 \end{tabular}
 \end{center}
L'abscence d'individus qui émergent de la sous-parcelle EH s'explique par $\mu_{\text{EH}}$ qui est fixé à 0.046, ce qui veut dire que 95\% des larves meurent en essayant de s'enfouir dans la sous-parcelle ainsi que 95\% des femelles issues de pupaison ou de diapause qui essayent d'en émerger.
...
Cependant, la qualité d'ajustement des dynamiques est pareille que précedemment : grossièrement captée pour les sous-parcelles PS et EH et mauvaise pour la sous-parcelle ER.
...
0 & 0.968 & 1 & 0.025 & 0.1 & 14483 & 6.26
 \end{tabular}
 \end{center}
 Ici aussi, il y a une abscence totale d'individus qui émergent de la sous-parcelle avec un enherbement haut ($\mu_{\text{EH}} = 0.025$)
 L'absence d'individus exogènes se compensent par un nombre d'œufs pondus plus élevé que précedemment (6.26 contre 3.08) et un stock de larves diapausante significatif (\texttt{stock} $=14483$).
...
0.026 & 0.93 & 0.565 & 0.647 & 0.174 & 13823 & 4.818
 \end{tabular}
 \end{center}
On observe que l'émergence d'individus dans la sous-parcelle EH ($\mu_{\text{EH}} = 0.647$ contre 0 précedemment) a entrainé une baisse d'émergence des individus dans la sous-parcelle ER ($\mu_{\text{ER}} = 0.565$ contre 0.97 et 1 pour les deux solutions précédentes)
...
Ces trois scénarios semblent tous montrer que le modèle actuel n'est pas suffisament complet pour capter le phénomène observé.
...
Il y a dans le modèle actuel un problème de décroissance en fin de saison.
...
Et si en testant une de ces hypothèses, on trouve une solution qui permet d'ajuster convenablement les dynamiques, on pourra alors essayer de valider la solution sur le verger n\textdegree2.
...
\begin{figure}[h]
 ...
 \epsfig{file = plots/B1.eps, scale = 0.46}
 
 \textbf{Scénario 2}
 
 \epsfig{file = plots/B2.eps, scale = 0.46}
 
 \textbf{Scénario 3}
 
 \epsfig{file = plots/B3.eps, scale = 0.46}
 \caption{Dynamiques observées et simulées pour chacun des trois scénarios. La décomposition indiquant la provenance des femelles qui ont pondus les œufs est disponible pour les dynamiques simulées.}
 \label{fig:B}
\end{figure}
...
Puisque la température n'explique pas le phénomène observé en fin de saison, on s'intéresse aux ressources.
...
Afin de pouvoir tester cette hypothèse au sein de notre modèle, il faut être capable de simuler des dynamiques d'inflorescences attractives.
Pour cela, on a besoin des débourrements des inflorescences (voir annexe~\ref{chap:deb}) ainsi que de la loi de mortalité des inflorescences.
On suppose que la durée de vie d'une inflorescence suit une loi normale.
Le \emph{dataset 1} contient les durées de vie effective des inflorescences.
En fixant le risque de première espèce à $\alpha = 5\%$, un test de normalité de Shapiro--Wilk nous confirme la normalité ($p$-valeur de 0.05447).
Ensuite, on observe une durée de vie effective moyenne de 29 jours (avec un écart-type de 14).

Ces inflorescences \emph{attractives} peuvent alors se calculer en utilisant la formule
\[
I_{t}^{a} = B_t + \sum_{t = 1}^{d_A} B_{t-j} \times \left( 1 - F(j) \right), \qquad \text{ avec } B_{t} = 0 \text{ si } t \leq 0. 
\]
Ici, $B_t$ représente le nombre de débourrements à la date $t$, $d_A$ la durée d'attractivité et $F$ la fonction de répartition d'une $\mathcal{N}\left( 29; 14 \right)$.

Nous ferons ici l'hypothèse que la durée d'attractivité est de seize jours ($d_A = 16$), ce qui correspond à la durée théorique des stades phénologiques C, D et E.
La différence entre les inflorescences vivantes et attractives est visible sur la figure~\ref{fig:CDE}.
...
Les deux premiers scénarios correspondent à ceux trouvés précedemment.
...
\begin{figure}[ht]
 ...
 \epsfig{file = plots/C1.eps, scale = 0.46}
 
 \textbf{Scénario 2}
 
 \epsfig{file = plots/C2.eps, scale = 0.46}
 
 \textbf{Scénario 3}
 
 \epsfig{file = plots/C3.eps, scale = 0.46}
...
On remarquera que la prédiction sur le troisième scénario (également présent sur la figure~\ref{fig:C}) est toujours aussi mauvaise.

La prise en compte de l'attractivité des inflorescences ne permet toujours pas la baisse des dynamiques observées en fin de saison.
...
On émet l'hypothèse qu'un phénomène intervient à cette date.
...
Pour tester cette hypothèse, on introduit dans le donc un paramètre de saisonnalité $\xi \in [0;1]$ sur le nombre de femelles dans le verger à partir du 15 septembre.
